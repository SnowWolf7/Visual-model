/**
 * Created by SnowWolf7 on 04/05/2018.
 */
...
/*
 函数功能：解析文本，获取tags.
 应用：parseText(d3.select(#text).property("value"))


 */
function parseText() {
    d3.csv("file/data.csv",function (data) {
    	tags = data;
    });
    console.log(tags);
    generate()
...
//called with two arguments:an array of word objects that were successfully placed, and a bounds object of the form[{x0,y0},{x1,y1}] representing the extent of the placed objects
function draw(t, e) {
...
var form = d3.select("#form").on("submit", function() {
	parseText(), d3.event.preventDefault()
});
// console.log(form);
/*
	click.refresh事件监听器中function内容需要学习
 */
